...
const std::string saveFilePath = "../../../CheckoutStorage";
const std::string sendFilePath = "../Storage/NosqlDb";
...
bool Sender::sendFile(Message msg)
{
	if (!msg.containsKey("file"))
		return false;
	std::string fileSpec = msg.value("sendFilePath") +"/" + msg.file();
...
public:
	...
	*/
	bool receiveFile(Message msg)
	{
		...
		std::string fileSpec = msg.value("saveFilePath") + "/" + fileName;
