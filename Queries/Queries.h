...
* Build Process:
---------------------
*  CL /EHsc Queries.cpp  ../DateTime/DateTime.cpp ../XmlDocument/XmlDocument/XmlDocument.cpp
...
namespace NoSqlDb {
	...
	template<typename P>
	DbQueries<P> &DbQueries<P>::selectdateTime(DateTime dt1, DateTime dt2) {
		dbKeyList updatedkeys;
		DbCore<P> db = database();
	...
	DbQueries<P> &DbQueries<P>::selectCategories(const std::string &patrn)
	{
		...
		{
			int flag = 0;
			DbElement<P> dbele = db[catekey];
			P insStruct = dbele.payLoad();
	...
	DbQueries<P> &DbQueries<P>::ChldrnSpcfdKey(const std::string keyspfd)
	{
		...
		if (!db.contains(keyspfd))
			return *this;
		Children  chldrn = db[keyspfd].children();
	...
	DbQueries<P> & DbQueries<P>::selectIntersection(int n_args, ...)
	{
		...
		for (int i = 0; i < n_args; i += 2)
		{
			...
			it = std::set_intersection(keyset1.begin(), keyset1.end(), keyset2.begin(), keyset2.end(), common.begin());
			if (common.size() > 0)
