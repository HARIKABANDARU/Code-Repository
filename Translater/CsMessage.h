...
namespace MsgPassingCommunication
{
  ...
  public:
	...
	//----------------< Add a new attribute to Message >----------
    void add(Key^ key, Value^ value)
    {
      attributes->Add(key, value);
    }
	//----------------< Extract a value from the attribute dictionary atructure of message  >-
	Value^ value(Key^ key)
	{
	...
	//-------------------< Show the message recived/send by the client using Translater >-----
    void show()
    {
      Console::Write("\n  CsMessage:");
	...
	Dictionary<String^,String ^>^ process1()
	{
		
		Dictionary<String^, String^>^ formattributes = gcnew Dictionary<String^, String ^>();;
		auto enumer = attributes->GetEnumerator();
		while (enumer.MoveNext())
		{
			String^ key = enumer.Current.Key;
